...
import { User, initializeAuthModels, RoleModel, UsuarioModel, MateriaModel }
    from "./orm_auth_models";
import { AuthStore, MateriaResult, Role } from "./auth_types";
...
export class OrmAuthStore implements AuthStore {
    ...
    async initModelAndDatabase(): Promise<void> {
        ...
        await this.storeOrUpdateUser(
            "Eber", "Flores", "Vazquez", "holaquehace", "eber.flores@alumno.buap.mx", "202119501"
        )

        await this.storeMateria(
            "Matematicas", "Basico", "Ciencias basicas", 6
        )
    ...
    async getUser(username: string) {//recupera credenciales buscando por su nombre
        return await UsuarioModel.findByPk(username);
    }
    ...
    async storeOrUpdateUser(nombre: string, apellidoPa: string, apellidoMa: string, password: string, correo: string, matricula: string) {
        const salt = randomBytes(16); //se genera salt
        const contraseña = await this.createHashCode(password, salt);//se hashea password
        const [model] = await UsuarioModel.upsert({
    ...
    async storeMateria(nombre: string, nivel: string, area: string, creditos: number){
        const [model] = await MateriaModel.upsert({
    ...
    async getMaterias(limit: number): Promise<MateriaResult[]> {
        return await MateriaModel.findAll({
            limit,
            order: [["id", "ASC"]]
        })
    }
    ...
    async validateCredentials(username: string, password: string):
        ...
        if (storedCreds) {
            ...
            return timingSafeEqual(candidateHash, storedCreds.contraseña);
